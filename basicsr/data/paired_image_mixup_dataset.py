...
from basicsr.utils.binary_transform import *
...
class PairedImageMixupDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        img_lq_1, img_gt_1, lq_path_1, gt_path_1 = self.getitem(index)
        index_2 = np.random.randint(self.__len__()-1)
        img_lq_2, img_gt_2, lq_path_2, gt_path_2 = self.getitem(index_2)
        # weight = np.random.rand()
        if random.random() < 0.5:
            weight = 1
        else:
            weight = np.random.uniform(0.2, 0.8)
        img_lq = weight * img_lq_1 + (1 - weight) * img_lq_2
        img_gt = weight * img_gt_1 + (1 - weight) * img_gt_2
        return {'lq': img_lq, 'gt': img_gt, 'lq_path': lq_path_1+' '+lq_path_2, 'gt_path': gt_path_1+' '+gt_path_2}
    ...
    def getitem(self, index):
        ...
        # index = torch.randperm(img_gt.shape[0])
        # img_gt = img_gt[index, ...]
        # img_lq = img_lq[index, ...]
        # normalize
        ...
        return img_lq, img_gt, lq_path, gt_path  #{'lq': img_lq, 'gt': img_gt, 'lq_path': lq_path, 'gt_path': gt_path}
