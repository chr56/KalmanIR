...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        if flag:
            ...
            xs = torch.cat([x_H, x_H_flip], dim=1) # (1, 4, 192, 3136)
            ks = torch.cat([k_H, k_H_flip], dim=1).view(B, K, -1, L) # (1, 4, 192, 3136)
        else:
            # x_H = x.view(B, -1, L)
            # x_H_flip = torch.flip(x_H, dims=[-1])
            ...
            xs = torch.cat([x_V, x_V_flip], dim=1) # (1, 4, 192, 3136)
            ks = torch.cat([k_V, k_V_flip], dim=1).view(B, K, -1, L) # (1, 4, 192, 3136)
        flag = not flag
        # print("----------", flag)
...
class MambaIR_pd_V4(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MambaIR_pd_V4, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
