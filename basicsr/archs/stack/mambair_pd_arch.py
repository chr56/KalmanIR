...
class MambaIR_pd(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MambaIR_pd, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
    ...
    def forward_features(self, x):
        ...
        x0 = x
        x, k = self.patch_embed(x) # N,L,C

        x = self.pos_drop(x)

        for layer in self.layers:
            x, k = layer(x0, k, x, x_size)
        # print("x shape:", x.shape, "k shape:", k.shape)
        x = self.norm(x)  # b seq_len c
        x = self.patch_unembed(x0, k, x, x_size)
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x0, k, x, x_size):
        xx, k = self.patch_embed(self.conv(self.patch_unembed(x0, k, self.residual_group(x, x_size), x_size)))
        return xx + x, k
...
class PatchEmbed(nn.Module):
    ...
    def DT(self, x):
        ...
        for i in range(c):
            dIdx[:, i, :, 1:] = torch.abs(dIcdx[:, i, :, :])
            dIdy[:, i, 1:, :] = torch.abs(dIcdy[:, i, :, :])
            kx[:, i, :, 1:] = dIcdx[:, i, :, :]
            ky[:, i, 1:, :] = dIcdy[:, i, :, :]
...
class PatchUnEmbed(nn.Module):
    ...
    def IDT(self, x, k, x_DT, x_size):
        ...
        for i in range(self.embed_dim//2):
            dIdx[:, i, :, :] = torch.cumsum(x_IDT[:, i, :, :], dim=2)
            dIdy[:, i, :, :] = torch.cumsum(x_IDT[:, i + 1, :, :], dim=1)
