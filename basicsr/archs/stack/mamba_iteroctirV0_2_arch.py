...
class MambaITEROCTIRV0_2(nn.Module):
    r""" MambaIR Model
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MambaITEROCTIRV0_2, self).__init__()
        num_in_ch = in_chans
        ...
        self.last_mamba = SS2DLast(d_model=2*num_out_ch)
    ...
    def forward(self, x):
        ...
        var = torch.sum(self.kl_loss(x_decimal_1, binary_to_decimal(b_BCE)), dim=1)
        x_var = x_decimal_1 * torch.mean(torch.exp(-var))

        # fuzy_area = self.last_norm(torch.square(b_L1-b_BCE))
        # x = x[:, :24, ...] * (1+fuzy_area)
        x = torch.cat((b_L1, b_BCE), dim=1)
        x = self.last_mamba(x)
        x_binary_2 = torch.sin(x)
        x_decimal_2 = binary_to_decimal(x_binary_2) + x_var
