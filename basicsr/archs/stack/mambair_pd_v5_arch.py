...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        if (flag % 4) == 1:
            ...
            xs = x_H_flip.view(B, K, -1, L)
            ks = k_H_flip.view(B, K, -1, L)
        else:
            x_V, k_V = self.DT(x.transpose(2, 3).contiguous())
            x_V_flip = torch.flip(x_V, dims=[-1])
            k_V_flip = torch.flip(k_V, dims=[-1])
            xs = x_V_flip.view(B, K, -1, L)
            ks = k_V_flip.view(B, K, -1, L)
        flag = flag + 1
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y1, flag = self.forward_core(x)
        assert y1.dtype == torch.float32

        if ((flag-1) % 4) == 1:
            y = self.IDT(x, y1)
        elif ((flag-1) % 4) == 2:
            y = self.IDT(x.transpose(2, 3).contiguous(), y1)
            y = y.transpose(2, 3).contiguous()
        elif ((flag-1) % 4) == 3:
            y = torch.flip(y1, dims=[-1])
            y = self.IDT(x, y)
        else:
            y = torch.flip(y1, dims=[-1])
            y = self.IDT(x.transpose(2, 3).contiguous(), y)
            y = y.transpose(2, 3).contiguous()
        y = y.permute(0, 2, 3, 1)
        y = torch.clamp(y, 0, 1)
...
class MambaIR_pd_V5(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MambaIR_pd_V5, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
