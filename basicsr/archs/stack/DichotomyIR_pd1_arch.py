...
class SS2D_g(nn.Module):
    ...
    def DT(self, x):
        ...
        x = torch.cat((dHdx, dVdy), dim=1)
        k = torch.cat((kx, ky), dim=1)
        return x, k

    def IDT(self, x, K, x_DT):
        b, c, h, w = x.shape
        dIdx = torch.zeros(b, c, h, w).to(x.device)
        dIdy = torch.zeros(b, c, h, w).to(x.device)
        x_IDT = x_DT - 1
        x_IDT = x_IDT * K
        # k = torch.where(x_IDT >= 0.0, torch.tensor(1.0), torch.tensor(-1.0))
        for i in range(1):
            dIdx[:, i, :, :] = torch.cumsum(x_IDT[:, i, :, :], dim=2)
            dIdy[:, i, :, :] = torch.cumsum(x_IDT[:, i + 1, :, :], dim=1)
        x_H = x[:, :, :, 0].unsqueeze(3).expand(b, c, h, w) + dIdx
        x_V = x[:, :, 0, :].unsqueeze(2).expand(b, c, h, w) + dIdy
        x_fuse = x_H + x_V
...
class DichotomyIR_pd1(nn.Module):
    r""" MambaIR Model
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(DichotomyIR_pd1, self).__init__()
        num_in_ch = in_chans
        ...
        self.mamba_g = SS2D_g(d_model=num_out_ch * 2)
        self.mamba_b = SS2D_b(d_model=num_out_ch * 2)
    ...
    def forward(self, x):
        ...
        # print("---------------------------- x_decimal_2 min:", x_decimal_2.min().item(), "x_decimal_2 max:", x_decimal_2.max().item())
