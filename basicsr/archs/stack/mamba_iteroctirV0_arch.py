...
class MambaITEROCTIRV2_1(nn.Module):
    r""" MambaIR Model
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MambaITEROCTIRV2_1, self).__init__()
        num_in_ch = in_chans
    ...
    def forward(self, x):
        ...
        x_decimal_1 = binary_to_decimal(torch.sin(x[:, :24, ...]))#[bx24xhxw]
        x_binary_1 = torch.sigmoid(x[:, 24:, ...])
        fuzy_area = self.last_norm(torch.square(torch.sin(x[:, :24, ...])-x_binary_1))
        x = x[:, :24, ...] * (1+fuzy_area)
        x = self.last_mamba(x)
