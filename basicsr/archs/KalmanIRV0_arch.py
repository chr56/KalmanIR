...
from .kalman.kalman_refine_network_v0 import KalmanRefineNetV0


@ARCH_REGISTRY.register()
class KalmanIRV0(nn.Module):
    r""" MambaIR Model
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(KalmanIRV0, self).__init__()
        # define IR model
        ...
        self.kalman_refine = KalmanRefineNetV0(dim=out_chans)
    ...
    def forward(self, x):
        ...
        factor_dz = calculate_difficult_zones_factor(binary_refined, binary_a, binary_b)

        dz_sequence = generate_difficult_zones_sequence(factor_dz, binary_refined, binary_a, binary_b)

        refined_difficult_zone = self.kalman_refine(dz_sequence)

        binary_refined = refined_difficult_zone + (1 - factor_dz) * binary_refined
...
def calculate_difficult_zones_factor(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor):
    factor_dz = torch.div(torch.abs(a - b) + torch.abs(b - c), 2)
    factor_dz = torch.tanh(factor_dz)
    return factor_dz


def generate_difficult_zones_sequence(
        factor_dz: torch.Tensor, a: torch.Tensor, b: torch.Tensor, c: torch.Tensor
):
    a_dz = torch.mul(a, factor_dz)
    b_dz = torch.mul(b, factor_dz)
    c_dz = torch.mul(c, factor_dz)
    return torch.stack((a_dz, b_dz, c_dz), dim=1)
