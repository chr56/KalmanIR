...
class PSA(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        kv = self.kv(x).reshape(b_, self.permuted_window_size[0], 2, self.permuted_window_size[1], 2, 2,
                                self.squeeze_dim).permute(0, 1, 3, 5, 2, 4, 6).reshape(b_, n // 4, 2, self.num_heads,
                                                                                       self.squeeze_dim * 4 // self.num_heads).permute(
            2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        # keep the channel dimension of Q: (num_windows*b, num_heads, n, c//num_heads)
        q = self.q(x).reshape(b_, n, 1, self.num_heads, self.squeeze_dim * 4 // self.num_heads).permute(2, 0, 3, 1, 4)[
            0]
        q = q * self.scale
