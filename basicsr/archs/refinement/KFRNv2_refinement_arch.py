...
class KFRNv2(nn.Module):
    def __init__(
        ...
        predictor_input_format =  self.kalman_predictor.model_input_format()
        assert len(predictor_input_format) == 2, f"Incompatible predictor input format: {predictor_input_format}"

    def preprocess_image_sequence(self, x: torch.Tensor):
    ...
    def forward(self, images: List[torch.Tensor]) -> dict:
        ...
        if self.input_order:
            images = [images[i] for i in self.input_order]

        if self.norm_image:
            images = [self.image_norm(image) for image in images]

        images = self.preprocess_image_sequence(torch.stack(images, dim=1))  # [B, L, C, H, W]
...
def perform_kalman_filtering(
    ...
    :param difficult_zone: difficult zone, shape [Batch, Channel, Height, Weight]
    ...
    for i in range(image_sequence_length):
        ...
        if i == 0:
            ...
            # Predict Step
            current_prime = predictor(previous.detach(), difficult_zone)
            # Update Step
