...
@MODULES_REGISTRY.register()
class DifficultZoneEstimatorV4final(nn.Module):
    def __init__(
            self,
            channels: int,
            num_images: int,
            num_block: int = 3,
            expand: int = 2,
            norm_type: NormLayerType = 'instance',
            base_activation: ActivationFunction = 'leaky_relu',
            last_activation: ActivationFunction = 'silu',
        ...
        for i in range(self.num_block):
            activation = last_activation if i == self.num_block - 1 else base_activation
            self.conv_layers.append(
                ResidualConvBlock(
                    in_channels=ch_mid, out_channels=ch_mid,
                    activation_type=activation, norm_type=norm_type,
                )
            )
