...
from .kalman.kalman_refine_network import KalmanRefineNetV0
...
class DichotomyKalmanV0(nn.Module):
    r""" MambaIR Model
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(DichotomyKalmanV0, self).__init__()
        num_in_ch = in_chans
        ...
        self.kalman_refine = KalmanRefineNetV0(dim=24)
    ...
    def forward(self, x):
        ...
        difficult_zones, eazy_zones = split_difficult_zones(
            (binary_refined, binary_a, binary_b)
        )

        refined_difficult_zone = self.kalman_refine(
            torch.stack(difficult_zones, dim=1)
        )

        binary_refined = eazy_zones[0] + refined_difficult_zone

        del difficult_zones, eazy_zones, refined_difficult_zone
...
def split_difficult_zones(
    ...
    factor_ez = 1 - factor_dz

    a_dz = torch.mul(a, factor_dz)
    b_dz = torch.mul(b, factor_dz)
    c_dz = torch.mul(c, factor_dz)

    a_ez = torch.mul(a, factor_ez)
    b_ez = torch.mul(b, factor_ez)
    c_ez = torch.mul(c, factor_ez)
