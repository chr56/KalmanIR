...
from .modules_mamba import BasicLayer, SS2D_g, SS2D_b
from .modules_common_ir import PatchEmbed, PatchUnEmbed, Upsample, UpsampleOneStep



'''
Fourier Loss 和 L1 Loss生成的2张图，转换为8进制后，用mamba改善不确定性。
'''

@ARCH_REGISTRY.register()
class DichotomyIRV4(nn.Module):
    r""" MambaIR Model
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(DichotomyIRV4, self).__init__()
        num_in_ch = in_chans
        num_out_ch = 3 #in_chans*8
        num_feat = 64
    ...
    def forward(self, x):
        ...
        x_L1 = torch.sin(x[:, :3, ...])
        x_Fourier = torch.sin(x[:, 3:, ...])

        for i in range(1):
            if i == 0:
                sigma = self.cal_kl(x_L1, x_Fourier)
                sigma_t = self.mamba_g(sigma)
                bias = self.mamba_b(x_L1)
                x_tp1 = (x_L1 / torch.exp(-sigma_t)) + bias
                x_t = x_L1
            else:
                sigma = self.cal_kl(x_tp1, x_t)
                sigma_t = self.mamba_g(sigma)
                bias = self.mamba_b(x_tp1)
                x_t = x_tp1
                x_tp1 = (x_tp1 / torch.exp(-sigma_t)) + bias

        x_refine = torch.sin(x_tp1)
