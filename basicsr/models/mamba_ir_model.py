...
from basicsr.utils.img_util import calculate_borders_for_chopping, recover_from_patches
from basicsr.utils.registry import MODEL_REGISTRY
from .base_model import BaseModel


@MODEL_REGISTRY.register()
class MambaIRModel(BaseModel):
    """MambaIR model for image restoration."""

    def __init__(self, opt):
        super(MambaIRModel, self).__init__(opt)

        # define network
        self.net_g = build_network(opt['network_g'])
        self.net_g = self.model_to_device(self.net_g)
        if opt.get('print_model_arch', True):
    ...
    def test(self):
        ...
        slices = calculate_borders_for_chopping(
            ral, row, split_h, split_w, shave_h, shave_w
        )
